...
$(function(){
	...
	$.ajax({
		type:"GET",//请求类型
		url:path+"/jsp/user/RoleListUser",//请求的url
	...
	userCode.bind("blur",function(){
		...
		$.ajax({
			type:"post",//请求类型
			url:path+"/jsp/user/findByuserCodeUser",//请求的url
	...
	picture.on("focus",function(){
		validateTip(picture.next(),{"color":"#666666"},"* 请上传证件照",false);
	}).on("blur",function(){
		if(picture.val() != null && picture.val() != ""){
			validateTip(picture.next(),{"color":"green"},imgYes,true);
		}else{
			validateTip(picture.next(),{"color":"red"},imgNo+" 上传大小不能超过 500 KB 上传文件类型必须为:jpg、png",false);
		}
	});
